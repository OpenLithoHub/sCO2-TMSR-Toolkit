...
|---|---|---|---|
| `Compressor.mo` (current scalar performance defaults) | engineering-typical values, not source-anchored | n/a | `eta_isen_design = 0.85`, `mdot_design = 100 kg/s`, `PR_design = 2.5` — used as fallback for tooling that doesn't exercise the table. Off-design φ → (ψ, η) lookup wired through `Modelica.Blocks.Tables.CombiTable1Dv`; default in-line table mirrors `validation/compressor_maps/sandia_main_compressor.csv`. Industrial users replace with their CSV via `tools/compressor_map_to_modelica.py` |
| `Compressor.mo` (wheel geometry block) | `Wright2010_SAND2010_0171` Table 5.1 (Confidence A, transcribed 2026-05-22) | `docs/data_extracts/wright2010_sand2010-0171.md` "Table 5.1 main-compressor wheel" | Now in `Compressor.mo` as named parameters: `r_tip`, `b2`, `beta_2b`, `Z_blades`, `blade_thickness`, `r_shroud_inlet`, `r_hub_inlet`, `beta_1bt`, `alpha_diff`, `tip_clearance`, `omega_design` |
| `Compressor.mo` (windage loss) | `Vrancik1968_NASA_TN_D4849` Eq. 5–6 (Confidence A, single-pass extracted 2026-05-22) | `docs/data_extracts/vrancik1968_nasa-tn-d4849.md` | `P_windage = π·C_d(Re)·ρ·r⁴·ω³·L_r` — implemented as opt-in branch (`enable_windage = true`); defaults `Cd_user = 0.03` (mid-range of Vrancik's Re=10⁴–10⁸ band) and laminar `C_d = 2/Re` fallback (`use_laminar_Cd`) exposed. 7 % experimental-error tolerance per Vrancik § "Experimental verification" (p.6) is the empirical band for downstream soft constraints. ReCompressor inherits this branch via `extends Compressor`. |
| `Turbine.mo` (current scalar defaults) | engineering-typical value | n/a | `eta_isen_design = 0.90` — symmetric BYOD interface (`useExternalMap` / `mapFileName`) added 2026-05-22 to mirror `Compressor.mo`, even though the off-design table lookup is a future deliverable. Windage not implemented (Vrancik's formula targets the rotor/alternator side per Wright2010 §5.4). |
...
| `IntermediateHeatExchanger.mo` chiller side (`cases/case04_chiller`) | `Wright2010_SAND2010_0171` Table 3.2 | same wright2010 extract doc | Tube OD 38.1 mm / wall 2.4 mm / coil 19.15 m — geometry seed for engineering-scale chiller benchmark; scaffold staged 2026-05-22 in `cases/case04_chiller/` (straight-tube placeholder; helical-coil mesh TODO) |
