...
**Status:** placeholder; `case04_chiller` engineering-scale helical-coil pipeline at production-grade refinement (CAD generator + surfaceFeatureExtract + snappyHexMesh (3 4)/(2 3) + surface layers); multi-region gas/liquid split scaffolded (`constant/regionProperties` declares target `(gas liquid)` topology) but not yet consumed by Allrun — promotion to chtMultiRegionFoam needs OpenFOAM-environment validation (see `cases/case04_chiller/README.md` "What still needs doing").
