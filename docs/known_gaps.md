...
**Status:** placeholder; wheel-geometry block + Vrancik windage closure source-anchored in `Compressor.mo` (2026-05-22). Off-design BYOD map (φ → ψ, η) wired through `Modelica.Blocks.Tables.CombiTable1Dv` for **both `Compressor.mo` and `Turbine.mo`** (2026-05-22) with in-line Sandia/centripetal-turbine placeholder defaults + shared CSV-to-Modelica converter (`src/tools/compressor_map_to_modelica.py` accepts `--table-name turbine_map`).
...
**Current placeholder:** `Components/Turbomachinery/{Compressor,ReCompressor,Turbine}.mo` exposes scalar isentropic-efficiency defaults plus a symmetric BYOD interface. `Compressor.mo` and `Turbine.mo` both drive `eta_isen` from a `CombiTable1Dv` keyed on flow coefficient `φ = ṁ/(ρ·ω·r_tip³)`; in-line default tables mirror `validation/compressor_maps/sandia_main_compressor.csv` (Wright2010 design point + generic centrifugal surge-to-choke shape, Confidence C) and `validation/turbine_maps/sandia_main_turbine.csv` (centripetal-turbine surge-to-choke shape around Wright2010's design φ band, Confidence C). When `useExternalMap=true`, the table is loaded from `mapFileName` (a `#1`-headed Modelica table .txt produced by `tools/compressor_map_to_modelica.py` — accepts `--table-name turbine_map` for the turbine variant). The scalar `eta_isen_design` / `mdot_design` / `PR_design` placeholders remain in the parameter block for tooling that does not exercise the table — they are explicitly tagged "not source-anchored". **Wheel geometry block (r_tip, b₂, β₂b, Z_r, blade thickness, r_s1, r_h1, β₁bt, α₂, tip clearance, ω_design) in `Compressor.mo` is Confidence-A from Wright2010 SAND2010-0171 Table 5.1 (transcribed 2026-05-22 verbatim from p.54).** Turbine wheel-geometry block exposes (r_tip, ω_design) only — Wright2010 publishes no Table-5.1 analogue for the turbine wheel. **Rotor-windage equation (Vrancik 1968 Eq. 5 with Re-dependent C_d closure, Confidence A) is wired into `Compressor.mo` as opt-in via `enable_windage`; defaults `C_d=0.03` (mid-range of Vrancik's Re=10⁴–10⁸ band) and laminar `C_d=2/Re` fallback exposed.** ReCompressor inherits both blocks + the BYOD table; Turbine inherits only the BYOD table (windage is rotor-loss specific to compressor/alternator side per Wright2010 §5.4).
...
**Status:** placeholder; `case04_chiller` engineering-scale helical-coil pipeline now **validated end-to-end in OpenFOAM v2512 (Docker `opencfd/openfoam-default:latest`, 2026-05-22)** through `blockMesh → snappyHexMesh → splitMeshRegions → checkMesh -region`. Smoke-test refinement (tube_wall (2 3) / shell_wall (1 2), surface layers off) yields **gas region 1.07 M cells, liquid region 2.35 M cells** with `gas_to_liquid` / `liquid_to_gas` regionCoupling patches auto-generated. Production knobs ((3 4) / (2 3) + 5/2 surface layers) preserved as commented-out alternatives in `system/snappyHexMeshDict` and require ≥ 16 GB container memory. Two structural improvements landed alongside this validation: (a) `helical_coil.py` now emits STL in **metres by default** (`--out-units m`) — snappyHexMesh consumes STL in raw units regardless of `blockMeshDict scale`, and the previous mm output sat 1000× outside the metres background mesh, refining zero cells; (b) `build_tube_stl(with_endcaps=True)` adds fan-triangulated end caps closing the swept ring (confirmed by `surfaceCheck`: "Surface is closed. All edges connected to two faces."), letting `refinementSurfaces.tube_wall` use `cellZone gas + cellZoneInside insidePoint` — far more robust than `locationsInMesh` flood-fill, which had been placing the gas seed in the same topological region as the liquid seed when the helix STL chopped the background into 11 disconnected pieces. Top-level `system/fvSchemes` + `system/fvSolution` stubs added so the mesh utilities resolve before per-region dictionaries take over. **Gas-region thermo** (Method-A polynomial fit, `icoPolynomial+hPolynomial+polynomial`, degree 7, generated by `src/tools/fit_thermo_polynomials.py` from CoolProp at P=7.7 MPa, T ∈ 320–360 K, rel-max errors < 1e-3 across rho/Cp/mu/k, sidecar at `cases/case04_chiller/constant/gas/sco2_thermo_coeffs.json`) carried over from the 2026-05-22 dict-only landing. **Limitation:** fit window stays above the pseudo-critical Cp spike near 305 K — the chiller outlet at ≈ 305 K is below the fit floor and needs Method B (custom sCO2Thermo class) for production accuracy. Next checkpoint: run `chtMultiRegionFoam` to first non-trivial time step.
...
**Current placeholder:** `cases/case0{1,2,3}_*/system/blockMeshDict` use idealized geometries from highly-cited public papers (Ngo et al., Kim et al.). Vendor comparison is explicitly out of scope. `cases/case04_chiller/` carries the SNL 10 MWe gas-chiller geometry from Wright2010 SAND2010-0171 Table 3.2 (Confidence A: tube ID 33.3 mm, single-coil length 19.15 m). The helical-coil CAD generator `src/tools/cad/helical_coil.py` (added 2026-05-22, tested in `tests/test_helical_coil_cad.py`) emits `helical_tube.stl` + `chiller_shell.stl` from those defaults; `system/surfaceFeatureExtractDict` extracts tube end-cap + shell-rim feature edges into `.eMesh` files; `system/snappyHexMeshDict` cuts both walls at production refinement (tube_wall (3 4) with 5 surface layers + cellZone `gas_zone` + faceZone `tube_wall`, shell_wall (2 3) with 2 surface layers) referencing those features. `splitMeshRegions -cellZones -overwrite` then peels the gas region (sCO₂ inside the tube) out of the default region (renamed `liquid` for the cooling-water shell side); the tube/shell interface is exposed as `tube_wall_gas` / `tube_wall_liquid` regionCoupling patches and uses `compressible::turbulentTemperatureCoupledBaffleMixed` for thermal matching. `chtMultiRegionFoam` then time-steps both regions in lockstep. Per-region thermo settings under `constant/{gas,liquid}/`: gas now uses **Method-A polynomial fits** (`icoPolynomial+hPolynomial+polynomial`, degree 7) generated by `src/tools/fit_thermo_polynomials.py` from CoolProp at P=7.7 MPa, T ∈ 320–360 K — replaces the prior `perfectGas+hConst` placeholder (2026-05-22). Liquid uses `rhoConst+hConst` water at 25 °C. **Not yet validated in an OpenFOAM environment** — see `cases/case04_chiller/README.md` "Validation status".
...
**Upstream (on disk, alternate):** `Wright2010_SAND2010_0171` Table 3.2 — engineering-scale gas-chiller coil geometry (tube OD 38.1 mm / wall 2.4 mm / coil 19.15 m). Now consumed by `cases/case04_chiller/` end-to-end (helical-coil STL via `src/tools/cad/helical_coil.py` + surfaceFeatureExtract + snappyHexMesh + splitMeshRegions cellZone tagging). **First OpenFOAM-environment validation completed 2026-05-22**: smoke-test mesh pipeline (1.07 M gas cells, 2.35 M liquid cells, regionCoupling patches generated) succeeds in OpenFOAM v2512 Docker. Next deliverable is the first `chtMultiRegionFoam` time step.
...
**Status:** broader-coverage second pass complete (six envelopes shipped: He @ 1/3/5 mol%, H₂O @ 0.5/1/2 mol%); CoolProp-version-bump regeneration workflow shipped as `validation/failure_envelopes/regenerate_all.sh` plus structured cell-flip diff at `validation/failure_envelopes/diff_status_codes.py` (working-tree vs. `--old`/`--git-ref`, exit code 0/1/2 for clean/flipped/grid-mismatch).
