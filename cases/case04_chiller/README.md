...
- [x] Replace gas-region thermo placeholder. `constant/gas/thermophysicalProperties`
      now uses `icoPolynomial+hPolynomial+polynomial` with degree-7
      coefficients fitted to CoolProp at P=7.7 MPa across **320–360 K**
      (above the pseudo-critical Cp peak — fit window deliberately
      brackets only the inlet side; outlet ≈ 305 K is *below* the fit
      floor and needs Method B). Generated by
      `python -m tools.fit_thermo_polynomials` — see
      `src/tools/fit_thermo_polynomials.py` and
      `tests/test_fit_thermo_polynomials.py`. Coefficient sidecar at
      `constant/gas/sco2_thermo_coeffs.json` gates fit quality
      (rel-max < 1e-3 across rho/Cp/mu/k).
